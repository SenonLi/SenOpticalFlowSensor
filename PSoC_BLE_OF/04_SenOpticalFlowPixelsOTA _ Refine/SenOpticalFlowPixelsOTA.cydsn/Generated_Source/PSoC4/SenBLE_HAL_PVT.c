...
{
    /* Setup ISR */
    SenBLE_uart_isr_StartEx(&CyBLE_Uart_Interrupt);

    SenBLE_HAL_Uart_Start();
}
...
void CyBLE_Uart_Stop (void)
{
    ...
    SenBLE_uart_isr_Stop();

    SenBLE_HAL_Uart_Stop();
}
...
{
    SenBLE_HAL_Uart_SpiUartPutArray(dataBuf, (uint32)length);
}
...
void CyBle_HalInit(void)
{
    ...
    SenBLE_bless_isr_SetVector(&CyBLE_Bless_Interrupt);
...
{
    SenBLE_bless_isr_StartEx(&CyBLE_Bless_Interrupt);
}
