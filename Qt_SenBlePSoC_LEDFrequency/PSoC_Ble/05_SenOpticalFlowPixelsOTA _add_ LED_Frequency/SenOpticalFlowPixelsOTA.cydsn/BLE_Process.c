        ...
        ***********************************************************/
		...
		case CYBLE_EVT_GATTS_WRITE_REQ: 
			...
			if(CYBLE_RGB_LED_RGB_LED_CONTROL_CHAR_HANDLE == wrReqParam->handleValPair.attrHandle) { 
                ...
                CYBLE_GATTS_HANDLE_VALUE_NTF_T		ledFrequencyNotificationHandle;	
            	uint8 ledColorArray[4];
            	ledColorArray[0] = RGBledData[RED_INDEX];
            	ledColorArray[1] = RGBledData[GREEN_INDEX];
            	ledColorArray[2] = RGBledData[BLUE_INDEX];
            	ledColorArray[3] = RGBledData[INTENSITY_INDEX];
                
        		/* Update notification handle with CapSense slider data*/
        		ledFrequencyNotificationHandle.attrHandle = CYBLE_SENMOTIONBURST_MOTIONBURST_CHAR_HANDLE;				
        		ledFrequencyNotificationHandle.value.val = &(ledColorArray[0]);
        		ledFrequencyNotificationHandle.value.len = 4u;
        		
        		/* Send the updated handle as part of attribute for notifications */
        		CyBle_GattsNotification(cyBle_connHandle,&ledFrequencyNotificationHandle);
