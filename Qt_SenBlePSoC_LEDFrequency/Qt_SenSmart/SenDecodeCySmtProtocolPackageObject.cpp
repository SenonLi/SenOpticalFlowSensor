...
//                evtFeedBackToConsole += getAddFeedBackStringFromAddStartPointer(pixels18Array, 19);
            ...
            }else if (evtPackageArray.length() == 10)  {   // LED Frequency

                QByteArray evtFeedBackToConsole;
                int index = 0;
                uchar deviceID, valueHandle_Low, valueHandle_High, red, green, blue;
                deviceID = (uchar)evtPackageArray[index++];
                evtFeedBackToConsole += "    Led Frequency: ";
                evtFeedBackToConsole += "\tDevice ID\t";
                evtFeedBackToConsole += getHexFeedBackStringFromDataStartPointer(&deviceID, 1);
                index++;// skip attID, useless
                evtFeedBackToConsole += "\tValue Handle   0x";
                ...
                index += sizeof(ushort);// skip Color Info length, useless

                evtFeedBackToConsole += "\tColor Now :\t\t\t";


                red = evtPackageArray[index++];
                green = evtPackageArray[index++];
                blue = evtPackageArray[index++];

                if (red == 0xff) {
                    evtFeedBackToConsole += "Red";
                    ledColor = LED_Color_Red;
                }else if(green == 0xff)  {
                    evtFeedBackToConsole += "Green";
                    ledColor = LED_Color_Green;
                }else if (blue == 0xff) {
                    evtFeedBackToConsole += "Blue";
...
//                if (isGattCommand == true && (int)cmdOpCodeID == (uchar)Cmd_Exchange_GATT_MTU_Size_Api && (int)evtStatus == 0)    emit emitGattCmd_DiscoverAllPrimaryServices();
                if (isGattCommand == true && (int)cmdOpCodeID == (uchar)Cmd_Exchange_GATT_MTU_Size_Api && (int)evtStatus == 0)    emit emitSL_GattCMD_LED_FrequencyServiceActive();//emit emitGattCMD_MotionServiceActive();
                if (isGattCommand == true && (int)cmdOpCodeID == (uchar)Cmd_Write_Characteristic_Value_Api && (int)evtStatus == 0)  {
                    if      (ledColor == LED_Color_Red)     emit emitSL_GattCMD_LED_WriteLedGreen();
                    else if (ledColor == LED_Color_Green)   emit emitSL_GattCMD_LED_WriteLedBlue();
                    else if (ledColor == LED_Color_Blue)    emit emitSL_GattCMD_LED_WriteLedRed();
                }
