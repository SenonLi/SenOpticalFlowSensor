...
//                                    lastCommandNotComplete = true;
    ...
    void SL_GattCMD_LED_FrequencyServiceActive()  {   sendCmd_Header();   sendGattCmd_WriteCharacteristicValue_Header();   sendGattCmd_WriteCharacteristicValue_LedFrequencyServiceHandle();  sendGattCmd_WriteCharacteristicValue_WriteLedBlue();   }
    void SL_GattCMD_LED_WriteLedRed()    {   sendCmd_Header();   sendGattCmd_WriteCharacteristicValue_Header();   sendGattCmd_WriteCharacteristicValue_LedFrequencyServiceHandle();  sendGattCmd_WriteCharacteristicValue_WriteLedRed();   }
    void SL_GattCMD_LED_WriteLedGreen()  {   sendCmd_Header();   sendGattCmd_WriteCharacteristicValue_Header();   sendGattCmd_WriteCharacteristicValue_LedFrequencyServiceHandle();  sendGattCmd_WriteCharacteristicValue_WriteLedGreen();   }
    void SL_GattCMD_LED_WriteLedBlue()   {   sendCmd_Header();   sendGattCmd_WriteCharacteristicValue_Header();   sendGattCmd_WriteCharacteristicValue_LedFrequencyServiceHandle();  sendGattCmd_WriteCharacteristicValue_WriteLedBlue();   }
...
////
    ...
    void slotSL_GattCMD_LED_WriteLedRed()   { SL_GattCMD_LED_WriteLedRed();   ledFrequencyCount++;}
    void slotSL_GattCMD_LED_WriteLedGreen() { SL_GattCMD_LED_WriteLedGreen(); ledFrequencyCount++;}
    void slotSL_GattCMD_LED_WriteLedBlue()  { SL_GattCMD_LED_WriteLedBlue();  ledFrequencyCount++;}
    void slotSL_GattCMD_LED_FrequencyServiceActive() { SL_GattCMD_LED_FrequencyServiceActive(); ledFrequencyCount = 1; ledFrequencyElapsedTimer->restart();  }
...
//                SEND_DONGLE_PACKET(getTestPacket_InputServiceRange());
    ...
    void sendGattCmd_WriteCharacteristicValue_Header()
            {   SEND_DONGLE_PACKET(dongleProtocol->getGattCmdPacket_WriteCharacteristicValue_Header());  }
    void sendGattCmd_WriteCharacteristicValue_LedFrequencyServiceHandle()
            {
                if (ledFrequencyCount >= 1000)  {
                    ledFrequencyRealTime = ledFrequencyElapsedTimer->elapsed();//milliseconds
                    float speed = (1000 / (float)ledFrequencyRealTime);
                    qDebug() << "BLE Speed:\t" << speed
                             << "\tmiliSec per time , \t\t"
                             << ledFrequencyRealTime << "\t miliSec for 1000 times communication";
                    ledFrequencyCount = 0;
                    ledFrequencyElapsedTimer->restart();
                }
                SEND_DONGLE_PACKET(dongleProtocol->getGattCmdPacket_WriteCharacteristicValue_LedFrequencyServiceHandle());  }
    void sendGattCmd_WriteCharacteristicValue_WriteLedRed()
            {   SEND_DONGLE_PACKET(dongleProtocol->getGattCmdPacket_WriteCharacteristicValue_WriteLedRed());  }
    void sendGattCmd_WriteCharacteristicValue_WriteLedGreen()
            {   SEND_DONGLE_PACKET(dongleProtocol->getGattCmdPacket_WriteCharacteristicValue_WriteLedGreen());  }
    void sendGattCmd_WriteCharacteristicValue_WriteLedBlue()
            {   SEND_DONGLE_PACKET(dongleProtocol->getGattCmdPacket_WriteCharacteristicValue_WriteLedBlue());  }

    /*************************************************************************************************************************/
    int senStopPreparationTime;

    QElapsedTimer *ledFrequencyElapsedTimer;//milliseconds
    qint64 ledFrequencyRealTime;//milliseconds
    int ledFrequencyCount;
